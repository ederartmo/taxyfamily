...
const pickupInput = document.getElementById('origen') || document.getElementById('pickup');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (btn) {
    btn.addEventListener('click', function() {
      if (navigator.geolocation) {
        btn.disabled = true;
        btn.innerHTML = '<img src="./ubi.png" alt="Ubicación actual" style="width:22px;height:22px;display:block;" />';
  ...
  // Detectar ubicación automáticamente al escribir si el campo está vacío
  if (pickupInput) {
    let ubicacionDetectada = false;
    pickupInput.addEventListener('focus', function() {
      if (!pickupInput.value && !ubicacionDetectada && navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
...
function setupAutocomplete(input) {
  const ac = new google.maps.places.Autocomplete(input, {
    ...
    // componentRestrictions: { country: "us" }, // Descomenta si lo necesitas
  });

  ac.addListener("place_changed", () => {
    const place = ac.getPlace();
    console.log("Evento place_changed", place);
    if (!place || !place.place_id) return;
    input.blur();
    setTimeout(() => {
      document.querySelectorAll(".pac-container").forEach(el => {
        el.style.display = "none";
        console.log("Ocultando .pac-container por place_changed");
      });
    }, 0);
  });

  input.addEventListener("blur", () => {
    console.log("Input perdió el foco", input);
    document.querySelectorAll(".pac-container").forEach(el => {
      el.style.display = "none";
      console.log("Ocultando .pac-container por blur");
    });
  });
  ...
  input.addEventListener("keydown", (e) => {
    if (e.key === "Escape") {
      ...
      document.querySelectorAll(".pac-container").forEach(el => {
        el.style.display = "none";
        console.log("Ocultando .pac-container por Escape");
      });
    }
  });
}
...
const placesApiKey = window.PLACES_API_KEY || '';
...
const translations = {
  es: {
    ...
    wa: {
      ...
      origin: 'Origen',
      destination: 'Destino',
      originMapLink: 'Ubicación origen (Google Maps)',
      destinationMapLink: 'Ubicación destino (Google Maps)',
  ...
  en: {
    ...
    wa: {
      ...
      originMapLink: 'Origin location (Google Maps)',
      destinationMapLink: 'Destination location (Google Maps)',
...
function applyLanguage(lang) {
  ...
  setText('label[for="origen"]', t.pickupLabel);
...
function buildGoogleMapsLink(lat, lng, fallbackAddress) {
  if (typeof lat === 'number' && typeof lng === 'number') {
    return `https://www.google.com/maps/search/?api=1&query=${lat},${lng}`;
  }

  const query = (fallbackAddress || '').trim();
  if (!query) {
    return '-';
  }
...
async function fetchPlacePredictions(inputText, sessionToken) {
  ...
  const response = await fetch(`https://api.familytaxinyc.com/api/places-autocomplete?${params.toString()}`);
...
async function fetchPlaceDetails(placeId, sessionToken) {
  const paramsDetails = new URLSearchParams({
    place_id: placeId,
    sessionToken
  });
  const response = await fetch(`/api/place-details?${paramsDetails.toString()}`);
...
function initPlacesAutocompleteNew(input, onSelected) {
  if (!input || !placesApiKey) {
    return;
  }

  const datalistId = `${input.id}-places-list`;
  ...
  input.addEventListener('input', () => {
    ...
    debounceId = window.setTimeout(async () => {
      const predictions = await fetchPlacePredictions(text, sessionToken);
      datalist.innerHTML = '';

      predictions.forEach((prediction) => {
        const option = document.createElement('option');
        option.value = prediction.text.text;
  ...
  input.addEventListener('change', async () => {
    const selectedOption = Array.from(datalist.options).find((option) => option.value === input.value.trim());

    if (!selectedOption || !selectedOption.dataset.placeId) {
      return;
    }

    const details = await fetchPlaceDetails(selectedOption.dataset.placeId, sessionToken);
    if (!details) {
      return;
    }
...
document.querySelectorAll('[data-action="next"]').forEach((button) => {
  button.addEventListener('click', () => {
    // Forzar cierre de autocompletado y blur de inputs
    if (typeof pickupInput !== 'undefined' && pickupInput) pickupInput.blur();
    if (typeof destinationInput !== 'undefined' && destinationInput) destinationInput.blur();
    document.querySelectorAll('.pac-container').forEach(el => el.style.display = 'none');
    if (typeof window.closeActivePlacesDropdown === 'function') {
      window.closeActivePlacesDropdown();
    }
    if (!validateStep()) {
      return;
    }
...
initPlacesAutocompleteNew(pickupInput, (details) => {
  pickupLat = details.lat;
  pickupLng = details.lng;
});

initPlacesAutocompleteNew(destinationInput, (details) => {
...
confirmWhatsAppBtn.addEventListener('click', () => {
  ...
  const text = [
    ...
    `${t.wa.originMapLink}: ${buildGoogleMapsLink(typeof pickupLat !== 'undefined' ? pickupLat : null, typeof pickupLng !== 'undefined' ? pickupLng : null, bookingState.pickup)}`,
    `${t.wa.destination}: ${bookingState.destination}`,
    `${t.wa.destinationMapLink}: ${buildGoogleMapsLink(typeof destinationLat !== 'undefined' ? destinationLat : null, typeof destinationLng !== 'undefined' ? destinationLng : null, bookingState.destination)}`,
